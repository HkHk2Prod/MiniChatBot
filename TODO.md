...
- [x] Add tests (see "Unit test suite" below — do this on its own branch)
...
- [x] `tests/test_gsm8k_reward.py` — `minichatbot/rl/rewards/gsm8k.py`
...
- [x] `tests/test_registry.py` — `minichatbot/utils/registry.py`
...
- [x] `tests/test_eval_scoring.py` — `minichatbot/eval/scoring.py`
...
- [x] `tests/test_chat_template.py` — `minichatbot/chat/template.py`
...
- [x] `tests/test_config.py` — `minichatbot/config.py`
...
- [x] `tests/test_sampling.py` — `minichatbot/inference/strategies/*.py`
...
- [x] `tests/test_bpe_tokenizer.py` — `minichatbot/tokenizer/bpe.py`
...
- [x] `tests/conftest.py` — shared fixtures: tiny trained `BPETokenizer`, stub
  `Tokenizer` (implements the base interface), stub model returning fixed logits.
- [x] `.github/workflows/ci.yml` — run `pytest` + `ruff check` on push/PR
